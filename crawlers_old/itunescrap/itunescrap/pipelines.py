            ...
            if(item):
              ...
              if(movie_set):
                for row in movie_set:
                    ...
                    cur.execute(sql)
                     if(price_set):
                      cur.execute ("UPDATE tbl_prices SET video_url=%s, unique_id=%s, Rent_sd_price=%s, Rent_hd_price=%s, Buy_sd_price=%s, Buy_hd_price=%s, updated_date=%s  WHERE movie_id=%s AND vendor_id=%s", (item["Video_urls"],'0',0,0,0,0,'1',created_date,movie_id,Vendor_id))
                      ...
                      cur.execute(insert_sql,(movie_id,item["Video_urls"],'0',Vendor_id,0,0,0,0,'1',created_date,created_date))
              else:
                  if(item["Movie_title"]):
